...
| DISAGREE | 5 | API and SAT disagree on pass/fail |
...
- **API details:** `{'unencrypted_clusters': []}`
- **SAT says:** score=1 (FAIL)
- **SAT details:** `{"clusters":"all_interactive_clusters"}`
...
- **Severity:** Medium
- **API says:** score=0 (PASS)
- **API details:** `{'external_model_count': 1}`
- **SAT says:** score=1 (FAIL)
- **SAT details:** `{"model_serving_endpoints_external_model":"No model serving endpoints with endpoint type EXTERNAL_MODEL found"}`
...
- **API details:** `{'admin_count': 2, 'admins': ['sfe-team-members', 'SAT_SP']}`
...
These checks have validators but no corresponding result was found in the SAT `security_checks` table for run_id=24.
...
|----------|------|----------|----------|-----|-----|---------|
| DP-1 | Secrets management | Data Protection | Low | FAIL | FAIL | AGREE |
| DP-10 | Disable legacy DBFS root and mounts | Data Protection | Medium | PASS | PASS | AGREE |
| DP-11 | SQL warehouse results download disabled | Data Protection | Medium | FAIL | FAIL | AGREE |
| DP-13 | DBFS file browser disabled | Data Protection | Medium | FAIL | FAIL | AGREE |
| DP-14 | Store and retrieve embeddings securely | Data Protection | Low | FAIL | FAIL | AGREE |
| DP-2 | Cluster instance disk encryption | Data Protection | Low | PASS | FAIL | DISAGREE |
| DP-3 | Customer-managed keys for managed services and workspace storage | Data Protection | Low | FAIL | FAIL | AGREE |
...
| DP-9 | FileStore endpoint for HTTPS file serving | Data Protection | Medium | PASS | PASS | AGREE |
| GOV-10 | Managed tables in DBFS root | Governance | Low | PASS | PASS | AGREE |
...
| GOV-15 | Enable verbose audit logs (on Azure, diagnostic logs) | Governance | Medium | PASS | PASS | AGREE |
...
| GOV-36 | Automatic cluster update | Governance | Medium | FAIL | FAIL | AGREE |
...
| GOV-5 | Deprecated versions of Databricks runtimes | Governance | High | PASS | PASS | AGREE |
...
| IA-8 | PAT token creation restricted to admins | Identity & Access | Medium | PASS | PASS | AGREE |
...
| INFO-29 | Streamline the usage and management of various large language model(LLM) providers | Informational | Medium | PASS | FAIL | DISAGREE |
...
| INFO-6 | Number of admins | Informational | Low | FAIL | PASS | DISAGREE |
...
| NS-11 | Workspace IP access list enforcement enabled | Network Security | High | FAIL | FAIL | AGREE |
...
| NS-4 | Workspace uses a customer-managed VPC (AWS, GCP) or enables VNet injection (Azure) | Network Security | Medium | FAIL | FAIL | AGREE |
