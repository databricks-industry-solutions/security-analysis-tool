...
dfexist.filter(
    (dfexist.analysis_enabled == True) & (dfexist.connection_test == True)
).createOrReplaceTempView("all_workspaces")
...
workspacedf = spark.sql(
    "select * from `all_workspaces` where workspace_id='" + current_workspace + "'"
)
if len(workspacedf.take(1)) == 0:
    dbutils.notebook.exit(
        "The current workspace is not found in configured list of workspaces for analysis."
    )
display(workspacedf)
...
if response.status_code == 200:
    ...
    dbutils.notebook.exit(
        "Invalid access token, check configuration value for this workspace."
    )
...
BODY = {
    "path": f"{basePath()}/[SAT] Security Analysis Tool - Assessment Results.lvdash.json"
}
...
BODY = {
    "display_name": "[SAT] Security Analysis Tool - Assessment Results",
    "warehouse_id": json_["sql_warehouse_id"],
    "serialized_dashboard": json_string,
    "parent_path": f"{basePath()}/dashboards",
}
...
if exists != True:
    ...
    response = requests.post(
        URL, headers={"Authorization": "Bearer %s" % token}, json=BODY, timeout=60
    )
