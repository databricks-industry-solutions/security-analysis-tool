...
client_credential = dbutils.secrets.get(json_['master_name_scope'], json_["client_secret_key"])
    ...
    for ws in workspaces:
        ...
        if ( ('workspaceId' in ws['properties']) and (((workspace_id is None) and (str(ws['properties']['workspaceId']) != current_workspace)) 
            ...
            if 'workspaceUrl' in ws['properties']:
                ...
                if token:
                    storeTokenAsSecret(workspace_url, workspace_pat_scope, tokenscope+"-"+str(ws['properties']['workspaceId']), ws_pat_token, token)
else:
    loggr.info(f"Error querying workspace API. Check account tokens: {response}")   
