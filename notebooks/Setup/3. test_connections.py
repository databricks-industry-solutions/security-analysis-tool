...
if cloud_type == "azure":  # use client secret
    ...
    json_.update(
        {"token": "dapijedi", "mastername": mastername, "masterpwd": masterpwd}
    )
else:  # lets populate master key for accounts api
    mastername = dbutils.secrets.get(
        json_["master_name_scope"], json_["master_name_key"]
    )
...
def test_connection(jsonarg, accounts_test=False):
    ...
    try:
        ...
        if is_successful_ws == True:
            ...
            loggr.info(
                f"Unsuccessful {hostname} {workspace_id} connection. Verify credentials."
            )
    except requests.exceptions.RequestException as e:
        is_successful_ws = False
        loggr.exception(
            f"Unsuccessful {hostname} {workspace_id} workspace connection. Verify credentials."
        )
    except Exception:
        is_successful_ws = False
...
if len(workspacesdf.take(1)) == 0:
    dbutils.notebook.exit(
        "Workspace list is empty. At least one should be configured for analysis"
    )
...
def modifyWorkspaceConfigFile(input_connection_arr):
    ...
    spark.createDataFrame(input_connection_arr, schema).createOrReplaceTempView(
        "incomsm"
    )
    ...
    dfmerge.toPandas().to_csv(
        f"{prefix}/workspace_configs.csv", mode="w", index=False, header=True
...
def renewWorkspaceTokens():
    if cloud_type == "gcp":
        ...
        gcp_status2 = dbutils.notebook.run(
            "../Setup/gcp/configure_tokens_for_worksaces", 3000
        )
        if gcp_status2 != "OK":
...
for ws in workspaces:
    ...
    clusterid = spark.conf.get("spark.databricks.clusterUsageTags.clusterId")
    json_.update(
        {"url": hostname, "workspace_id": workspace_id, "clusterid": clusterid}
    )
    ...
    if cloud_type == "azure":  # use client secret
        ...
        masterpwd = dbutils.secrets.get(
            json_["master_pwd_scope"], json_["master_pwd_key"]
        )
        json_.update({"token": token, "mastername": mastername, "masterpwd": masterpwd})
    ...
    if (json_["use_mastercreds"]) is False:
        ...
        try:
            ...
            loggr.info(
                f"\033[1m{hostname} workspace connection. Connection Status: {ret_tuple[1]}\033[0m"
            )
            continue
