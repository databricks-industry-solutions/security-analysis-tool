...
display(spark.sql(f"""insert into {json_["analysis_schema_name"]}.account_workspaces  (select workspace_id, deployment_url, workspace_name, workspace_status, analysis_enabled  from `all_workspaces` where workspace_id not in (select workspace_id from {json_["analysis_schema_name"]}.account_workspaces))"""))
