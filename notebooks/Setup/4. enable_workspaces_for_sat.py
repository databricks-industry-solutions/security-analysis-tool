...
# MAGIC insert into security_analysis.account_workspaces  (select workspace_id, deployment_url, workspace_name, workspace_status, ws_token, analysis_enabled, sso_enabled, scim_enabled, vpc_peering_done, object_storage_encrypted, table_access_control_enabled  from `global_temp`.`all_workspaces` where workspace_id not in (select workspace_id from security_analysis.account_workspaces)); 
