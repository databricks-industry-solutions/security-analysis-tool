...
if len(dfexist.take(1)) == 0:
    dbutils.notebook.exit(
        "Workspace list is empty. At least one should be configured for analysis and be accessible from current workspace"
    )
...
display(
    spark.sql(f"""select * from {json_["analysis_schema_name"]}.account_workspaces""")
)
