...
def generate_shared_run_id():
    ...
    def _run_sql_with_retry(sql, what, max_attempts=5):
        for attempt in range(max_attempts):
            try:
                return spark.sql(sql)
            except Exception as e:
                is_concurrency_conflict = any(
                    marker in str(e)
                    for marker in ("DELTA_METADATA_CHANGED", "ConcurrentAppendException", "ConcurrentWriteException", "MetadataChangedException")
                )
                if is_concurrency_conflict and attempt < max_attempts - 1:
                    backoff = 10 * (2 ** attempt)
                    loggr.info(f"Concurrent write conflict during {what}. Retrying in {backoff}s (attempt {attempt + 1}/{max_attempts})")
                    time.sleep(backoff)
                    continue
                raise
