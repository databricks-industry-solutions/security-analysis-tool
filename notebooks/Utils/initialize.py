...
account_id = _optional_secret("account-console-id").strip()
# Explicit sat_scope key, or implicit when account-console-id is absent/empty.
workspace_only = _optional_secret("workspace-only-mode").strip().lower() in (
    "true",
    "1",
    "yes",
) or account_id == ""
...
if cloud_type == "azure":
    ...
    json_.update(
        {
            "subscription_id": (
                _optional_secret("subscription-id")
                if workspace_only
