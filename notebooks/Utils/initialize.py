...
json_ = {
    "account_id": dbutils.secrets.get(scope="sat_scope_v3", key="account-console-id"),
    "sql_warehouse_id": dbutils.secrets.get(scope="sat_scope_v3", key="sql-warehouse-id"),
    "analysis_schema_name": dbutils.secrets.get(
        scope="sat_scope_v3", key="analysis_schema_name"
    ),
    "verbosity": "info",
    "proxies": json.loads(dbutils.secrets.get(scope="sat_scope_v3", key="proxies")),
}
...
json_.update(
    {
        ...
        "master_pwd_scope": "sat_scope_v3",
        "master_pwd_key": "pass",
        "workspace_pat_scope": "sat_scope_v3",
    ...
    json_.update(
        {
            "service_account_key_file_path": dbutils.secrets.get(
                scope="sat_scope_v3", key="gs-path-to-json"
            ),
            "impersonate_service_account": dbutils.secrets.get(
                scope="sat_scope_v3", key="impersonate-service-account"
            ),
    ...
    json_.update(
        {
            ...
            "subscription_id": dbutils.secrets.get(
                scope="sat_scope_v3", key="subscription-id"
            ),  # Azure subscriptionId
            "tenant_id": dbutils.secrets.get(
                scope="sat_scope_v3", key="tenant-id"
            ),  # The Directory (tenant) ID for the application registered in Azure AD.
            "client_id": dbutils.secrets.get(
                scope="sat_scope_v3", key="client-id"
...
if cloud_type == "aws":
    ...
    try:
        use_sp_auth = (
            dbutils.secrets.get(scope="sat_scope_v3", key="use-sp-auth").lower() == "true"
        )
        if use_sp_auth:
