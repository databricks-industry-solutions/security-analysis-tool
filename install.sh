...
aws_validation() {
  ...
  read -p "Enter Databricks Workspace URL: " DATABRICKS_URL
  ...
  read -p "Run SAT on Serverless? ('Yes' or 'No'): " RUN_ON_SERVERLESS
  while ! validate_run_on_serverless "$RUN_ON_SERVERLESS"; do
    echo "Invalid answer. It must be either 'Yes' or 'No'."
    read -p "Run SAT on Serverless? ('Yes' or 'No'): " RUN_ON_SERVERLESS
  done
  ...
  AWS_VAR_ARGS=(
    ...
    "proxies=$AWS_PROXIES"
    "run_on_serverless=$(convert_run_on_serverless "$RUN_ON_SERVERLESS")"
  )
...
azure_validation() {
  ...
  read -p "Enter Databricks Workspace URL: " AZURE_DATABRICKS_URL
