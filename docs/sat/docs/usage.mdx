...
Within the `Workflows` pane, there should now be three jobs available: **SAT Initializer Notebook**, **SAT Driver Notebook**, and **SAT Secrets Scanner Notebook**. 
...
Secret scan results are stored in the `secret_scan_results` table within your SAT schema. Each finding includes:
* Notebook path and ID
* Detector name (type of secret detected)
* SHA-256 hash of the detected secret (for security)
* Verification status
* Scan timestamp

Results can be queried directly or integrated into your existing security monitoring dashboards.
