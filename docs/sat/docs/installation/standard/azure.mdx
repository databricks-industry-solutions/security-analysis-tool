...
<Admonition type="info" title="Two Azure auth paths">
**Full account analysis (this guide):** Microsoft Entra app registration, subscription Reader, Databricks Account Admin, and Account ID. One SAT per Azure subscription can monitor all workspaces in that subscription.

**Skip Databricks account APIs:** In `./install.sh`, choose **Skip Databricks account APIs? (registers this workspace)**. Use a **Databricks-managed** service principal (`client-id` / `client-secret`) that is a workspace admin here. You do not need an Entra app, tenant ID, subscription ID, or Account ID. Checks that need Azure Management (NS-3, NS-4, NS-6) stay off. Optionally provide Entra tenant + subscription so **GOV-3** can run for this workspace. Details: [Skip Databricks account APIs](../../usage.mdx#skip-databricks-account-apis-workspace-only).
</Admonition>
...
📚 [Learn more about Service Principals](https://learn.microsoft.com/en-us/azure/databricks/admin/users-groups/service-principals#--databricks-and-microsoft-entra-id-formerly-azure-active-directory-service-principals)
...
**Full account analysis** — have these ready:
...
**Skip account APIs** — Databricks-managed service principal **Client ID** and **Client Secret** only. Skip Steps 1–2 (Entra app). Add that SP as workspace admin in this workspace (not Account Admin). If you want GOV-3, also provide tenant ID, subscription ID, and subscription Reader as in Steps 1–2.
</Admonition>
