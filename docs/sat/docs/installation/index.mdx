...
- Databricks Account ID (not required for [workspace-only mode](../usage.mdx#workspace-only-mode))
- Databricks SQL Warehouse (To run the SQL dashboard)
- PyPI access from your workspace (To install the SAT utility library)

<Admonition type="note" title="Workspace-only analysis">

To analyze only the install workspace as a workspace admin (no account APIs), `./install.sh` is unchanged. After install, set `sat_scope` key `workspace-only-mode` to `true`, then run the SAT Initializer and Driver. See [Workspace-only mode](../usage.mdx#workspace-only-mode).
