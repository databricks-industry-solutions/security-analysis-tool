...
* Set `skip_account_apis = true` to skip Databricks account APIs and register this workspace only. Leave `account_console_id` empty. For a Databricks-managed service principal, also leave `tenant_id` and `subscription_id` empty. For Entra (GOV-3 on this workspace), set **both** tenant and subscription IDs.
* Full Azure analysis (`skip_account_apis = false`) requires `account_console_id`, `tenant_id`, and `subscription_id`.

#### Service Principal Role Requirements:
* Full Azure analysis: "Reader" role at the subscription level via Access control (IAM).
* Full Azure analysis: [Accounts Admin role](https://learn.microsoft.com/en-us/azure/databricks/admin/users-groups/service-principals#--assign-account-admin-roles-to-a-service-principal)
* [Admin role](https://learn.microsoft.com/en-us/azure/databricks/admin/users-groups/service-principals#assign-a-service-principal-to-a-workspace-using-the-account-console) for **each workspace** (workspace-only: this workspace)
