...
- Account Denylist Builder (inactive-user group candidates + an Entra ID dynamic-group rule helper)
...
- **Entra ID dynamic-group rule helper** — a guided builder that emits [dynamic membership rule](https://learn.microsoft.com/en-us/entra/identity/users/groups-dynamic-membership) syntax (guest users, domain match, department, company, disabled accounts) so a single denylist entry can cover thousands of identities
