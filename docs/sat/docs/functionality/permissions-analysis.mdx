...
- **`07_denylist_candidates.py`** → `brickhound_denylist_candidates`. Ranks **IdP-managed (external)** account groups by inactive-member count (inactive = no `system.access.audit` activity in the look-back window — a heuristic for the Automatic Identity Management "Inactive: No usage" status). Feeds the **Account Denylist Builder** tab. **AIM caveat:** under Automatic Identity Management, external group memberships are resolved just-in-time and are not returned by the account SCIM Groups API, so member-based ranking can be empty even for populated groups; in that case use the Entra ID dynamic-group rule builder in the app tab instead.
:::
...
- Account Denylist Builder (inactive-user group candidates + an Entra ID dynamic-group rule builder)
...
### Detect Privileged Non-IdP Group Identities
