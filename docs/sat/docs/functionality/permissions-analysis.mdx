...
#### Configure User Authorization (recommended)

The app is intended for workspace and account admins. To make sure the
app cannot act as a privileged proxy for users beyond their own Unity
Catalog grants, configure user authorization so the app forwards each
caller's identity to Unity Catalog:

1. Open the app in the Databricks UI
   (**Compute → Apps → sat-permissions-exp**).
2. Click **Edit** → **User authorization**.
3. Click **+ Add scope** and select `sql` (used by the Statement
   Execution API to read the BrickHound tables).
...
> **Behavior during the migration.** If user authorization is not yet
> configured, the app falls back to its service principal identity and
> a one-time warning is logged. Functionality is unchanged. Configure
> user authorization at your convenience to enable per-user enforcement.
