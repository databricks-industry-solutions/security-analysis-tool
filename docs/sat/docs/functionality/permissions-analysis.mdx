...
| `05_share_to_account.py` | Detect & optionally remediate resources shared to all account users | "Which dashboards/Genie spaces/Apps were shared with everyone?" |
...
- **`07_denylist_candidates.py`** → `brickhound_denylist_candidates`. Ranks account groups by inactive-member count (inactive = no `system.access.audit` activity in the look-back window — a heuristic for the Automatic Identity Management "Inactive: No usage" status). Feeds the **Account Denylist Builder** tab.
:::
...
**Example Query:** List every dashboard, Genie space, and App shared with all account users in the last 30 days, and optionally revoke that access.
