...
:::note
`05_share_to_account.py` is audit-log based (reads `system.access.audit`) rather than graph based, so it does not require the data collection job to have run first. It writes findings to the `brickhound_shared_to_account` table and can be scheduled via the "SAT Permissions Analysis - Shared to Account Users" job. Detection is report-only by default; set the `remediate` parameter to `yes` to automatically remove the "account users" ACL entry (the SAT service principal must be a member of each affected workspace).
:::
